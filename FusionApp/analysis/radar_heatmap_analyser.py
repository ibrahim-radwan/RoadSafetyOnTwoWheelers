...
from sample_processing.radar_proc import openradar_pd_process_frame, pyradar_process_frame, openradar_rt_process_frame, custom_process_frame
...
class RadarHeatmapAnalyser(RadarAnalyser):
    ...
    def _preprocess_frame_from_raw_data(self, dca_frame: np.ndarray) -> np.ndarray:
        ...
        if self.adc_params is None:
            raise RuntimeError("ADC parameters not initialized. Call run() method first.")
    ...
    def _analyse_frame(self, dca_frame: DCA1000Frame) -> Dict[str, Any]:
        ...
        # Extract results
        range_doppler_matrix = result["range_doppler"]  # This will be None for openradar method
    ...
    def run(
        ...
        config_to_use = self.config_file if self.config_file else CFGS.AWR2243_CONFIG_FILE
        self.adc_params = ADCParams(config_to_use)
        self.logger.info(f"ADC parameters initialized from config file: {config_to_use}")
        ...
        try:
            ...
            if isinstance(item, dict) and "ADC_PARAMS" in item:
                self.logger.info("Skipping ADC_PARAMS from queue (using config file parameters)")
        ...
        self.logger.info(f"Initialized with {self.adc_params.tx} TX, {self.adc_params.rx} RX antennas")
        self.logger.info(f"Range Resolution: {self.adc_params.range_resolution:.4f} m")
        self.logger.info(f"Doppler Resolution: {self.adc_params.doppler_resolution:.4f} m/s")
