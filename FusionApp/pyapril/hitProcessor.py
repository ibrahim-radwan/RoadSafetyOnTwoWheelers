...
def target_DOA_estimation(rd_maps, hit_list, DOA_method, array_alignment):
    ...
    for hit in hit_list:
        azimuth_vector = rd_maps[:, hit[0], hit[1]]
        # print("Hit: ", hit, "Azimuth vector: ", azimuth_vector)
        ...
        if DOA_method == "Fourier":
            ...
            doa_res = de.DOA_LPM(R, scanning_vectors, element_select=0)
        elif DOA_method == "Bartlett":
            doa_res = de.DOA_Bartlett(R, scanning_vectors)
        else:
            raise ValueError("Unknown DOA estimation method: {}".format(DOA_method))
