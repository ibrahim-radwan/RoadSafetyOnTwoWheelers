...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Radar analysis disabled. ArrayFire and/or FFTW not found."
              << std::endl;
    std::cout << "To enable radar analysis:" << std::endl;
    std::cout << "1. Install ArrayFire: https://arrayfire.com/download/"
              << std::endl;
    std::cout << "2. Install FFTW3: sudo apt-get install libfftw3-dev"
              << std::endl;
    ...
    try
    {
        ...
        // Build ADC parameters directly from config file (includes derived
        // values)
        ...
        auto test_frame = std::make_shared<radar::RadarFrame>(
            std::chrono::duration_cast<std::chrono::seconds>(
                ...
                .count(),
            dummy_data, 1, adc_params);

        std::cout << "✓ Created test frame: " << test_frame->toString()
                  << std::endl;
        ...
        std::cout << "  Total time: " << duration.count() / 1000.0 << " ms"
                  << std::endl;
        std::cout << "  Frame timestamp: " << result.frame_timestamp
                  << std::endl;
        ...
        std::cout << "  Point cloud size: " << result.point_cloud.size()
                  << std::endl;
        if (result.range_doppler.elements() > 0)
        {
            af::dim4 rd_dims = result.range_doppler.dims();
            std::cout << "  Range-Doppler heatmap dims: (" << rd_dims[0] << ","
                      << rd_dims[1] << ")" << std::endl;
        }
        else
        {
            std::cout
                << "  Range-Doppler heatmap: None (as expected for stage1?)"
                << std::endl;
        }
        if (result.range_azimuth.elements() > 0)
        {
            af::dim4 ra_dims = result.range_azimuth.dims();
            std::cout << "  Range-Azimuth heatmap dims: (" << ra_dims[0] << ","
                      << ra_dims[1] << ")" << std::endl;
        }
        else
        {
            std::cout << "  Range-Azimuth heatmap: None" << std::endl;
        }
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            if (output_queue.waitAndPop(result, std::chrono::seconds(5)))
            {
                ...
                std::cout << "  Received result for frame "
                          << result.frame_number << " (processed in "
                          << result.processing_time_ms << " ms)";
                if (result.range_doppler.elements() > 0)
                {
                    af::dim4 rd = result.range_doppler.dims();
                    std::cout << " RD dims=(" << rd[0] << "," << rd[1] << ")";
                }
                std::cout << std::endl;
            }
            else
