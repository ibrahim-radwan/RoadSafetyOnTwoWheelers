...
# Import shared utilities for PNG file handling
from camera.png_utils import scan_png_directory, find_closest_frame_before
...
def create_video(
    ...
    # Use shared utility to scan directory for both camera and detection files
    camera_files, perspective_files = scan_png_directory(
        input_dir, include_detection_files=True
    )
    ...
    for perspective_path, perspective_timestamp, _ in perspective_files:
        # Find matching camera frame (returns tuple or None)
        camera_match = find_closest_frame_before(camera_files, perspective_timestamp)
        ...
        camera_path, _, _ = camera_match  # Extract filepath from tuple
