...
def range_based_pruning(det_obj_2d_raw,
    ...
    snr_idx3 = (det_obj_2d["SNR"] > snr_thresh[2, 1]) & (
        det_obj_2d["rangeIdx"] * range_resolution > snr_thresh[1, 0]
    )
    snr_idx = snr_idx1 | snr_idx2 | snr_idx3

    peak_val_idx = np.logical_not(
        (
            (det_obj_2d["peakVal"] < peak_val_thresh[0, 1])
            & (det_obj_2d["rangeIdx"] * range_resolution < peak_val_thresh[0, 0])
        )
        | (
            (det_obj_2d["peakVal"] < peak_val_thresh[1, 1])
            & (det_obj_2d["rangeIdx"] * range_resolution < peak_val_thresh[1, 0])
        )
    )
