...
def _apply_3d_cfar(
    ...
    arr_y: np.ndarray,
    arr_x: np.ndarray,
    guard_cell_zyx: Optional[Tuple[int, int, int]] = None,
    train_cell_zyx: Optional[Tuple[int, int, int]] = None,
    ...
    if guard_cell_zyx is None:
        guard_cell_zyx = _normalize_triplet(CFAR_GUARD_CELL_ZYX)
    if train_cell_zyx is None:
        train_cell_zyx = _normalize_triplet(CFAR_TRAIN_CELL_ZYX)
    if fa_rate is None:
        fa_rate = float(CFAR_FA_RATE)
